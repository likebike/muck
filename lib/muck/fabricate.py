...
# This file comes from https://github.com/SimonAlfie/fabricate
...
class StraceRunner(Runner):
    keep_temps = int(os.environ.get('FAB_KEEP_TEMPS', '0'))  ################  Edited by Christopher Sebastian: Changed from hard-coded 'False' to an environment lookup.
    ...
    def _do_strace(self, args, kwargs, outfile, outname):
        ...
        shell_keywords.update(kwargs)
        try:
            shell('strace', '-yfo', outname, '-e',        ##############  Edited by Christopher Sebastian: added 'y' flag.
    ...
    def _match_line(self, line, processes, unfinished):
        ...
        if execve_match:
            ...
            is_output = True
        elif getdents_match: match = getdents_match            ####################  Added by Christopher Sebastian
